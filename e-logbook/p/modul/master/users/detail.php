...
if($_POST['idx']) {
        $id = $_POST['idx'];      
        $sql = mysql_query("SELECT * FROM user u left join tdepartment d on u.id_dep=d.id_dep WHERE iduser = $id");
		...
		<input type='hidden' value='$r[iduser]' name='id'>
						...
						<div class='col-sm-6'>
							...
							if($r[level]=='admin'){
                               echo " <label class='fancy-radio custom-color-green'><input name='level' value='admin' type='radio' checked><span><i></i>Admin</span></label>
                                <label class='fancy-radio custom-color-green'><input name='level' value='user' type='radio'><span><i></i>User</span></label>";
                            }
							else{
								echo " <label class='fancy-radio custom-color-green'><input name='level' value='admin' type='radio'><span><i></i>Admin</span></label>
