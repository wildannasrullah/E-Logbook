                            ...
                            <div class='table-responsive'>
								...
								if($_GET[t]=='1'){
									$ro = mysql_query("SELECT a.*,mc.category_name,b.status FROM `tproblems` a 
													   ...
													   where a.idcat='$_GET[id]' and c.pic_handling='$_SESSION[username]' and b.status not in ('F','C') order by a.idprob desc");
								}else{
									$ro = mysql_query("SELECT a.*,mc.category_name,b.status FROM `tproblems` a 
															join tlaporan b on a.idprob=b.no_pelaporan 
															LEFT JOIN mcategories mc ON a.idcat=mc.idcat 
															join tassign c on a.idprob=c.no_pelaporan
															where c.pic_handling='$_SESSION[username]' and b.status not in ('F','C') order by a.idprob desc");
								}
									...
									echo "
                                        <tr>
											...
											<td>";
											$ro = mysql_query("SELECT STATUS from tlaporan where no_pelaporan='$r[idprob]'");
											$o = mysql_fetch_array($ro);
											if($o[STATUS]=="A"){
												echo "
												<a href='$aksi?p=todolist&act=update&st=IN&id=$r[idprob]'>
												<button type='button' class='btn btn-success btn-block'>IN PROGRESS</button></a></td>
												";
											} else if($o[STATUS]=="IN"){
												echo "
												<button type='button' class='btn btn-warning btn-block' data-toggle='modal' data-target='#finished' data-id='$r[idprob]'>FINISHED</button></td>
												";
											}
											echo "
										</tr>";
